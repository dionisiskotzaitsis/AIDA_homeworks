...
#or
heart_disease_2=select(heart_disease, -vars_to_remove$variable)
...
# dslb 1.1.2 Profiling categorical variables
# freq(): The output is ordered by the frequency 
# variable, which quickly analyzes the most
# frequent categories and how many shares 
# they represent (cummulative_perc variable).
...
profiling_num(data_world)

# select the most relevant metrics
my_profiling_table=profiling_num(data_world) %>% select(variable, mean, p_01, p_99, range_80)
# Printing only the first three rows
head(my_profiling_table, 3)

# Profiling numerical variables by plotting
plot_num(data_world)
plot_num(data_world, bins = 20)
...
ggplot(finches, aes(x = Depth, y = stat(density))) + 
  # plotting
...
p4 <- ggplot(df, aes(x)) +
  geom_histogram(binwidth = 5, center = 66.5,
                 fill = "lightBlue", col = "black") +
...
### GDP example
df <-read.csv("/home/gevan/data/countries2012.csv")
df %>% ggvis(~GDP) %>% 
...
# boxplots are NOT for categorical data!
library(likert) # data
